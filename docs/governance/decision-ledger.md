...
| **OD-17** | **Narrowed by DL-60/DL-61 review (2026-09-15); not closed.** Composite-fact aggregation algorithm and coverage thresholds — **but only for the Composite-Contributor configuration path**, since DL-60's Primary Authority selection already makes single-source, zero-aggregation consumption under **DL-59** the default for every Environmental Purpose (Temperature, Humidity, CO2, VOC, PM2.5, PM10, Noise, Leak, Pressure, Vibration, Lux, UV) whenever a household designates one. OD-17's algorithm is needed only where a household explicitly configures multiple Composite Contributors instead of a single Primary Authority, including for a Merged Room's own Primary-Authority-or-composite choice. **OD-17 additionally owns input-availability and coverage semantics for Formula-derived Facts** (Dew Point, Mold Index, Condensation Risk) — never their specific formula, which **DL-60** already assigns to implementation mapping. **Environmental Coverage, Configuration Completeness, and Room Environmental Stewardship Summary are explicitly not OD-17's** — they are **DL-61**-owned dashboard projections (simple ratios), never Composite Facts requiring an aggregation-algorithm decision; **OD-17's own "contributor coverage" (e.g. *2 of 3*) is a distinct concept from DL-61's "Environmental Coverage" and the two must never be conflated**. Consumes, without redefining, the Truth Confidence Band representation accepted as **DL-58**. **Contextual Person-Presence multi-evidence combination is not this decision's to make** — it is already accepted (DL-38 Identity fusion, consumed by Truth as one input); OD-17 governs environmental/device Composite Facts only, and consumes current-state acquisition and expiry from **DL-59** without redefining them. **Receives its Composite Contributor set, per Environmental Purpose, from Room Configuration's DL-60 disambiguation** (Primary Authority / Secondary / Composite Contributor / Excluded); OD-17 owns only the aggregation algorithm and coverage thresholds over that set, never which sources participate | Truth, Room Configuration | Composite fact computation |
...
| **OD-19** | Governed conflict-resolution rules for disagreeing evidence, **including disagreement between a natively assigned Area environmental entity and the HTBW-configured input set**, **and including a person-associated device conflict (for example a configured-stronger watch versus a configured-weaker phone reporting different Rooms), where person-specific configured Identity Evidence Association reliability applies and no universal device-type precedence is ever created**. **Uses the Truth Confidence Band representation accepted as DL-58** for any publishable resolved Fact, and the current-state/point-in-time distinction accepted as **DL-59**; does not borrow the Identity Fusion Function or DL-39 bands, and does not itself decide how conflict reduces confidence — that derivation remains OD-19's own to accept. **Receives only the current, valid, selected inputs Room Configuration's DL-60 disambiguation designates** (Primary Authority and Secondary/Composite sources); OD-19 does not choose primary authority and does not silently replace an explicit Room Configuration selection | Truth | Evidence conflict resolution |
