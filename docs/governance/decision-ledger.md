...
| **OD-88** | **Dynamic Capability Discovery and Room-Defined Presentation Outcomes** — how HTBW resolves, at delivery time, which currently available technical provider and evidence path satisfies a Room-defined presentation purpose when one physical endpoint may be represented by more than one provider (for example a Sonos speaker visible through the native Sonos integration, Music Assistant, and the Home Assistant device registry simultaneously), and how conflicting or partial evidence across providers, endpoints, and a multi-endpoint Room-defined outcome is resolved and explained. **This is distinct from, and does not reopen, Room Configuration's existing ownership of the static household-declared vocabulary-to-target mapping** (`docs/models/room-configuration.md` *Provides*: "Resolved target set for a given vocabulary term in a given Room") — that remains Foundation's, unchanged. **What has no owner is the runtime question**: which provider's evidence is authoritative for a specific capability when more than one provider describes the same physical endpoint; how a multi-endpoint Room-defined outcome's Presentation Outcome is determined when some endpoints succeed and others fail (**DL-54** provides the per-attempt outcome vocabulary; `failure-and-degradation.md`'s *"a configured speaker group is partially unavailable"* rule provides a starting precedent but does not extend it to a named room-level outcome-quality concept); how an artifact's interaction requirements are matched against a surface's currently available response-handling capability rather than assumed from remote possession or a touchscreen; and how one household-facing endpoint identity is associated with more than one technical provider record without merging on name, room, IP address, or vendor alone. **Bounded by what is already accepted and not reopened here**: **DL-41** stands, so a missing provider dependency is unavailable and named, never emulated; **DL-53** stands, so a Delivery Surface's capability declaration is Foundation-owned and household perceptibility declarations are unchanged; **DL-54** stands, so the Presentation Outcome vocabulary is not re-invented, only extended to a room-level, multi-endpoint aggregation; Room Configuration's vocabulary-to-target mapping is unchanged. **Alternatives to evaluate, none preferred here**: **(A)** a strict originating-provider-precedence rule per capability; **(B)** unresolved provider conflict always yielding Unknown for the affected determination; **(C)** room-level outcome quality expressed as a distinct projection field layered over, and never replacing, the endpoint-level DL-54 Presentation Outcomes; **(D)** endpoint identity association governed by an existing canonical mechanism rather than a new identity construct | Foundation, Truth, Concierge, Operational Trust | Dynamic capability discovery and room-defined presentation outcomes |
