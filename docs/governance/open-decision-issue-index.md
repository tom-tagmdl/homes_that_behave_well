...
| **OD-30** | Decision Trace Visibility | Open — Governance Required | **#80** | #79, #81, #133 (**resolved as DL-69**, related not blocking), #104 (**resolved as DL-72**, related not blocking) | DL-34, DL-36, DL-39, DL-46; **DL-69 confirms Conversation is one retrieval surface, audience-filtered; DL-72 confirms the default degradation order and its Decision Trace fields — OD-30 still owns the full surface enumeration** |
...
| **OD-57** | Indication and Content Separation | Open — Governance Required | **#102** | #104 (**resolved as DL-72**, related not blocking), #98, #119, #113 | DL-28, DL-29, DL-34; **DL-72 confirms Indication is the accepted degradation path and states illustrative safe/unsafe forms — OD-57 still owns the mandatory-versus-optional threshold** |
...
| **OD-71** | Closed — **DL-72**. **System-Initiated Communication** and **Person-Requested Response** are formally distinguished; an explicit request is evidence of willingness, never unrestricted consent. The accepted default degradation order under audience uncertainty: authorized private delivery → content-free Indication → neutral challenge → defer/refuse — is the privacy-preserving floor even absent configuration. Two Continuity-owned Person Setup preferences accepted: **Ordinary Delivery Preference** and the new **Audience-Uncertainty Routing Preference**, the latter supplying a desired degradation path Operational Trust must still permit. Authorized Private Delivery Surface criteria extended (Potential Perceptibility, availability, not-shared, not-merely-nearby, current device association, Presentation Outcome support) **without claiming any unverified lock-screen/possession/shared-device detection capability**. **Unresolved plurality, Audience unknown, Audience detection unavailable** (all **DL-34**'s existing states) and **contradictory evidence** (**DL-63**) remain four distinct, explainable diagnostic conditions sharing one disposition. Room and Merged Room receive identical treatment; content-classification enumeration stays **OD-51**'s and the Indication-mandatory threshold stays **OD-57**'s. Guest fallback and multi-caretaker rules consumed from **DL-71** without redefinition |
